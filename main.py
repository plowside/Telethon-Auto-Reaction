...
class sessions_manager:
	...
	async def task_executor(self, task):
		...
		while True:
			need_sessions_count = sessions_count - success
			if success < sessions_count:
				clients = [x for x in list(self.clients.items()) if x[1].id not in used_clients]
				if len(clients) == 0:
					logging.info(f'[{task_id_text}] Нехватает сесссий для выполнения задачи (Выполнено {success} из {sessions_count})')
					break
				clients = list(clients)[:need_sessions_count]
			else:
				break

			time_to_sleep_i = []
			temp = need_sessions_count
			temp3 = 0
			...
			for i, (client, client_data) in enumerate(clients, start=1):
				used_clients.append(client_data.id)
				if random.random() < chances['skip_session']:
					logging.info(f'[{task_id_text}|{client_data.id}] Пропустил задачу')
					continue
		...
		async with self.lock:
			...
			con.commit()
		logging.info(f'[{task_id_text}] Задача завершена')
